...
public class Tree extends LandscapeObject{
	...
	private Color trunkColor; 				// The Hex color of the trunk, including the preceding '#'. Set in the constructor.
	private Color branchColor; 			// The Hex color of the branches, including the preceding '#'. Set in the constructor.
	...
	public Tree(Graphics2D g2, int x, int y, double scale, int levels,  String s_trunkColor, String s_branchColor) {
		super(g2, x, y, scale);
		this.levels = levels;
		this.trunkColor = new Color(Integer.parseInt(s_trunkColor,16));// color trunk
		this.branchColor = new Color(Integer.parseInt(s_branchColor,16)); // color branch
	...
	private void drawBranches() {
		for (int levels = 0; levels < this.levels; levels++) {	// the level of triangles)
			g2.setColor(branchColor);// color 16
			g2.setStroke(treeStroke);// (1)
			int [] x = {(int) (super.currentX + (0.5 * trunkWidth) - (0.5 * branchWidth)), (int) (super.currentX + (0.5 * trunkWidth)), 
					(int) (super.currentX + (0.5 * trunkWidth) + (0.5 * branchWidth))};
			int [] y = {super.currentY, super.currentY - branchHeight, super.currentY}; // position of the branches
			g2.fillPolygon(x, y, 3);
			super.currentY = (int) (super.currentY - (branchOverlap * (branchHeight * 1.0)));// separation of the branches
			
			g2.setStroke(treeStroke);// (1)
			g2.setColor(Color.BLACK);
			g2.drawPolygon(x, y, 3);// position
		}
